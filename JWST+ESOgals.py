...
if ask_yes_no('Skip searching for galaxies and just use list made before?') == False:
    ...
    for sec in ['A2', 'A6', 'E1', 'A10', 'A1', 'A9', 'A4', 'A8', 'A7', 'A3', 'A5']:
        ...
        for band in ['Y','H','Ks']:
            pics.update({band: fits.getdata('fits/UVISTA_GB/UVISTA_'+band+'_Image.fits',ext=0)})
        w = WCS(fits.getheader('fits/nircam/'+sec+"/mosaic_nircam_f150w_COSMOS-Web_60mas_"+sec+'_v0_5_i2d.fits.gz',ext=1))
        w2 = WCS(fits.getheader('fits/UVISTA_GB/UVISTA_H_Image.fits',ext=0))
        for i in range(len(sections[sec])):
            num_images  = 6
            num_cols    = 3
            num_rows    = 2
            # Create a grid of subplots.
            fig, axes = plt.subplots(num_rows, num_cols, figsize=(17,10.25))
            list_axes = list(axes.flat)
            for n,band in enumerate(['115','150','277','Y','H','Ks']): # for each object
                if band in ['115','150','277']:
                    xpix, ypix = w.all_world2pix(sections[sec]['RAJ2000'][i],sections[sec]['DEJ2000'][i],1) # convert from RA/DEC to pixel position
                if band in ['Y','H','Ks']:
                    xpix, ypix = w2.all_world2pix(sections[sec]['RAJ2000'][i],sections[sec]['DEJ2000'][i],1) # convert from RA/DEC to pixel position
                xpix = round(float(xpix))
                ypix = round(float(ypix))
                if band in ['115','150','277']:
                    rpix = int(abs(round(float(sections[sec]['radFlux'][i]*5.89))))
                if band in ['Y','H','Ks']:
                    rpix = int(abs(round(float(sections[sec]['radFlux'][i]*2.38)))) # COSMOS pix * pix/deg = deg / (pix/deg) = pix radius
                ...
                if band in ['Y','H','Ks']:
                    list_axes[n].imshow(pics[band][down:up,left:right],cmap='gray',vmin=0,vmax=7.25,origin='lower',extent=[0,1,0,1],aspect='auto')
                ...
                if band in ['Y','H','Ks']:
                    list_axes[n].set_title(f'{sec}_{sections[sec]['Classic'][i]}_{band}')
